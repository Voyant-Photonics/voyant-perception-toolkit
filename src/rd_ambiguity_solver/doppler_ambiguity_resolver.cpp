...
AmbiguitySolver::AmbiguitySolver(const std::string &yaml_path) : pc_utils_(), ego_solver(pc_utils_)
{
...
Params AmbiguitySolver::loadParams(const std::string &yaml_path)
{
    ...
    try
    {
        ...
        if (!sensor_config)
        {
            throw std::runtime_error("[+] There seems to be an issue with the YAML file. "
        ...
        solver_params_.min_ego_velocity_for_correction = sensor_config["min_ego_velocity_for_correction"].as<double>();
        solver_params_.ransac_preference_threshold = sensor_config["ransac_preference_threshold"].as<double>();
...
std::pair<std::vector<double>, std::vector<double>> AmbiguitySolver::recoverRangeDoppler(std::vector<double> amb_rng,
                                                                                         std::vector<double> amb_dop)
{
                                                ...
                                                const std::vector<std::array<double, 12>> &points)
{
    ...
    // Write header
    file << "x,y,z,doppler,point_idx,frame_idx,timestamp,drop_reason,snr_linear,calibrated_reflectance,noise_mean_"
...
bool AmbiguitySolver::validatePointCoordinates(const PointDataWrapper &pt)
{
    if (std::isnan(pt.x()) || std::isnan(pt.y()) || std::isnan(pt.z()) || std::isinf(pt.x()) || std::isinf(pt.y()) ||
        std::isinf(pt.z()) || (pt.x() == 0.0 && pt.y() == 0.0 && pt.z() == 0.0))
    {
        return false;
    }
    ...
    if (std::abs(pt.x()) > MAX_COORD || std::abs(pt.y()) > MAX_COORD || std::abs(pt.z()) > MAX_COORD)
    {
        return false;
    }
    return true;
...
std::vector<std::array<double, 12>> AmbiguitySolver::Solver(const VoyantFrameWrapper &frame, size_t frame_id)
{
    std::vector<double> xs, ys, zs, vels, snrs, calibrated_reflectances, noise_mean_estimates, min_ramp_snrs;
    ...
    if (!xs.empty())
    {
        ...
        double rnsc_v_ego = ego_solver.p_ransac_ego_velocity(az, el, vels, config.rnsc_confidence, config.rnsc_thres,
                                                             config.rnsc_max_iter, config.rnsc_inlier_ratio);

        // Get top elevation's ego
        double top_ele_v_ego = ego_solver.getTopEleEgo(vels, az, el, config.VERTICAL_RES_DEG, config.num_pts_init);

        // Get the simple median ego velocity
        std::vector<double> ego_velocities;
        double med_v_ego;
        std::pair<double, std::vector<double>> med_estimator = ego_solver.simpleMedianEstimator(vels, cos_az, cos_el);
        ...
        if (med_diff >= config.ego_recollect_threshold && rnsc_diff >= config.ego_recollect_threshold)
        {
            ...
            // were noisy
            top_ele_v_ego = ego_solver.getTopEleEgo(vels, az, el, config.VERTICAL_RES_DEG, config.num_pts_recollect);
        }
        ...
        else if (med_diff <= config.ransac_preference_threshold && rnsc_diff <= config.ransac_preference_threshold)
        {
            inlier_v_ego = rnsc_v_ego;
        ...
        if (!amb_rng.empty())
        {
            ...
            for (size_t j = 0; j < amb_indices.size(); ++j)
            {
                ...
                std::array<double, 12> row = {nx[j], ny[j], nz[j],
                                              new_dop[j],                                  // Use j for recovered data
                                              static_cast<double>(pnt_idxs[original_idx]), // Use
                                                                                           // original_idx
                                                                                           // for metadata
                                              static_cast<double>(frame_id),
                                              static_cast<double>(timestamps[original_idx]),
        ...
        std::cout << "[+] Time taken to process a frame: " << duration.count() / 1e6 << " seconds" << std::endl;
    }
...
int main(int argc, char *argv[])
{
    ...
    auto end = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
    std::cout << "[+] Time taken to read the file: " << duration.count() / 1e6 << " seconds" << std::endl;
