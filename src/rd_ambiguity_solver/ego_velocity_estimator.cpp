...
double EgoInlierEstimator::p_ransac_ego_velocity(const std::vector<double> &azimuth,
                                                 ...
                                                 double confidence = 0.99, double threshold = 0.5,
    ...
    if (inliers > best_inliers) {
      ...
      if (inlier_ratio > rnsc_inlier_ratio) {
        ...
        if (prob_all_outliers > 0.0) {
          max_iterations = std::min(max_iterations, static_cast<int>(std::log(1.0 - confidence) /
                                                                     std::log(prob_all_outliers)));
        }
      }
    }
...
std::pair<double, std::vector<double>> EgoInlierEstimator::simpleMedianEstimator(
    const std::vector<double> &doppler, const std::vector<double> &cosaz,
